...
export const AuthContext = createContext({} as {
    ...
    keypair: ({ question1, question2, question3, question4, question5, email, HMAC }: { question1: string; question2: string; question3: string; question4: string; question5: string; email: string; HMAC: string; }) => Promise<void>;
    signup: ({ name, user, email, eddsaPublicKey }: { name: string; user: string; email: string; eddsaPublicKey: string; }) => Promise<void>;
});
...
export const AuthProvider = ({ children, publicPage = false }: any) => {
    ...
    return (<AuthContext.Provider value={{ authenticated, user, loading, logout, login, authenticationProcess: isAuthenticationProcess(), register, signup, keypair }}>
        {loading ?
            <div>Loading...</div>
            :
            <ApolloProvider client={createApolloClient(authenticated)}>
                {children}
            </ApolloProvider>
        }
