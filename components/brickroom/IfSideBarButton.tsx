...
const IfSideBarButton = ({
    ...
    const linkElement = (<span className={buttonClasses}>
                <div className="flex items-center flex-1">
                    {svg}
                    {text}
                </div>
            {tag && <span
                className="inline-flex items-center justify-center px-1 py-0.5 text-xs rounded-lg font-display text-primary bg-accent">
                    {tag}
                </span>}
        </span>)
