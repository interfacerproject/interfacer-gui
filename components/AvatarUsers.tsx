...
const AvatarUsers = ({ users }: { users: Array<{ displayUsername: string; id: string }> }) => {
  const pruneDuplicates: (u: { displayUsername: string; id: string }[]) => { displayUsername: string; id: string }[] = (
    u: Array<{ displayUsername: string; id: string }>
  ) => {
    return u.filter(
      (value, index, self) =>
        index === self.findIndex((t) => t.id === value.id && t.displayUsername === value.displayUsername)
    );
  };
  return (
