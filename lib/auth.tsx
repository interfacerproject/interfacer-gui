...
import { ApolloProvider, ApolloClient, InMemoryCache, HttpLink, gql, concat } from "@apollo/client";
...
const headersMiddleware = setContext(async (operation, { headers }) => {
  ...
  const query: any = operation.query.loc?.source.body!;
  const completeHeaders: any = await SignRequest({ query, variables }).then(({ result, logs }) => {
...
function useProvideAuth() {
  ...
  useEffect(() => setAuthUsername(storedAuthUsername), [storedAuthUsername]);
  ...
  useEffect(() => setAuthEmail(storedAuthEmail), [storedAuthEmail]);
  ...
  useEffect(() => setAuthName(storedAuthName), [storedAuthName]);
  ...
  keypairoomServer(firstRegistration: ${firstRegistration}, userData: "{\\"email\\": \\"${email}\\"}")
        ...
        `;
    return await client
      .mutate({ mutation: KEYPAIROOM_SERVER_MUTATION })
      .then(({ data }) => data)
      .catch((error) => {
  ...
  const signIn = async ({ email }: { email: string }) => {
    ...
    const result = await client
      .query({
        ...
        variables: {
          email,
          pubkey: getItem("eddsa_public_key", "local"),
        },
      })
      .then(({ data }) => {
